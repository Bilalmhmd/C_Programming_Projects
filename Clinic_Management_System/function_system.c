...
struct Patient_Node *head = NULL_PTR;
uint32 availableSlots = 5;    // No.available slots
uint32 arr[5] = {1,2,3,4,5};  //array of available slots
uint32 i ;
...
void EditPatientInfo_Func(void)
{
	...
	if (count == 0)
	{
		...
		printf("-------------------------------------------------------------------------------------------------\n");
		EditPatientInfo_Func(); //return again to the main function
	}
...
void ReserveSlot_Func(void)
{
	...
	else
	{
		...
		ReserveSlot_Func(); //return again to the main function
	}
}
	...
	else
	{
		...
		while(Ptr1 != NULL_PTR)
		{
			if(Ptr1->id == ID)
			{
				...
				availableSlots++;
				printf("              ************** Cancel The Reservation Done ****************");
				return;
			}
			else
			{
		...
		/*If The Admin Entered Wrong ID Print A Message To Re Enter The Right ID */
		printf(" Incorrect ID Please Re Enter The Right ID :\n");
		CancelReservation_Func(); // return again to the main function
	}
}
...
void User_Mode(void)
{
	...
	while(1)
	{
		...
		default :
			printf("                     ********* Invalid Choice ********* \n");
			printf("             ********* Please Choose The Right Action ********* \n");
			User_Mode(); //return again to the main function
			break;
		}
	}
}
...
void ViewPatientInformation_Func(void)
{
	...
	else
	{
		...
		printf("Incorrect ID Please Re Enter The Right ID : \n");
		ViewPatientInformation_Func(); //return again to the main function
	}
}
...
void ViewPatientsReservations_Func(void)
{
	...
	uint32 slot;
	Ptr1 = head;
	if(Ptr1 == NULL_PTR ) //check global variable to sure that did The Reservations Cancel ?
	{
		/* Check If There Is Patients Informations In The Database Or Not If Not Print This Message For The User */
